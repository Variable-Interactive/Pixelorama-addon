...
func update_brush() -> void:
	match _brush.type:
		Brushes.PIXEL:
			_brush_texture.create_from_image(load("res://addons/pixelorama/assets/graphics/pixel_image.png"), 0)
		Brushes.CIRCLE:
			_brush_texture.create_from_image(load("res://addons/pixelorama/assets/graphics/circle_9x9.png"), 0)
		Brushes.FILLED_CIRCLE:
			_brush_texture.create_from_image(load("res://addons/pixelorama/assets/graphics/circle_filled_9x9.png"), 0)
...
func _set_pixel(position : Vector2) -> void:
	...
	if _mask[i] < global.get_tools().pen_pressure:
		_mask[i] = global.get_tools().pen_pressure
		_drawer.set_pixel(image, position, tool_slot.color, global)
