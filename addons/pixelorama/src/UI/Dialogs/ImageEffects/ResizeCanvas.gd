...
func _on_ResizeCanvas_about_to_show() -> void:
	image = Image.new()
	image.create(get_node("/root/Pixelorama").current_project.size.x, get_node("/root/Pixelorama").current_project.size.y, false, Image.FORMAT_RGBA8)
	image.lock()
	var layer_i := 0
	for cel in get_node("/root/Pixelorama").current_project.frames[get_node("/root/Pixelorama").current_project.current_frame].cels:
		if get_node("/root/Pixelorama").current_project.layers[layer_i].visible:
			...
			image.blend_rect(cel_image, Rect2(get_node("/root/Pixelorama").canvas.location, get_node("/root/Pixelorama").current_project.size), Vector2.ZERO)
		layer_i += 1
	image.unlock()

	if first_time:
		width_spinbox.value = get_node("/root/Pixelorama").current_project.size.x
		height_spinbox.value = get_node("/root/Pixelorama").current_project.size.y
	update_preview()
...
func _on_WidthValue_value_changed(value : int) -> void:
	width = value
	x_spinbox.min_value = min(width - get_node("/root/Pixelorama").current_project.size.x, 0)
	x_spinbox.max_value = max(width - get_node("/root/Pixelorama").current_project.size.x, 0)
...
func _on_HeightValue_value_changed(value : int) -> void:
	height = value
	y_spinbox.min_value = min(height - get_node("/root/Pixelorama").current_project.size.y, 0)
	y_spinbox.max_value = max(height - get_node("/root/Pixelorama").current_project.size.y, 0)
...
func update_preview() -> void:
	...
	preview_image.blend_rect(image, Rect2(Vector2.ZERO, get_node("/root/Pixelorama").current_project.size), Vector2(offset_x, offset_y))
